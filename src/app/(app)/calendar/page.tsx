...
export default async function CalendarPage() {
  ...
  const dayDocs = await diary
    .find({ userId, dateKey: { $regex: `^${yyyymm}` } })
    .toArray();
  ...
  return (
    <div className="flex h-full flex-col overflow-hidden">
      <header className="flex shrink-0 items-center justify-between border-b border-border px-6 py-3">
        <h1 className="text-[18px] font-semibold tracking-tight">{grid.label}</h1>
      ...
      <div className="scroll flex-1 overflow-auto p-8">
        <div className="mx-auto max-w-[920px]">
          <div className="grid grid-cols-7 gap-2 px-1 pb-2 text-[11px] font-medium uppercase tracking-wide text-muted-foreground">
            ...
            {grid.cells.map((c, i) => {
              ...
              return (
                <Link
                  key={c.key}
                  href={href}
                  className={`min-h-[88px] rounded-[var(--radius)] border p-2 text-[12px] transition-colors ${
                    isToday
                ...
                >
                  <div className="flex items-center justify-between">
                    <span className="font-mono text-[13px] font-semibold">{c.day}</span>
                    {entry && entry.analyzedAt && (
                      <span className="h-1.5 w-1.5 rounded-full bg-chart-3" />
                    )}
                  </div>
                  {entry && (
                    <div className="mt-1 line-clamp-3 text-[11.5px] leading-tight text-muted-foreground">
